...
#' 
#' @description
#' Stepwise variable selection by AIC, for S4 regression object (e.g. \link[lme4]{glmer}).
#' 
...
stepAIC4 <- function(
  ...
  step.results <- function(models, fit, object, usingCp = FALSE) {
    ...
    # fit$anova <- aod # was; wont work for S4
    attr(fit, which = 'anova') <- aod # instead
    fit
  }
  
  Terms <- terms(object) # wont contain random terms!!
  #object$formula <- Terms # just remove
